...
class LabelFileDataset(Dataset):
    def __init__(self,
        ...
        for i, fname in enumerate(self._image_fnames):
            if not os.path.exists(fname): #E.g. it is only the filename, not the full path, assume to use same dir as labels file as base
                full_path = os.path.join(os.path.dirname(self._path), 'train', fname)
                if os.path.exists(full_path):
                    full_fnames.append(full_path)
            else:
                full_fnames.append(fname)
        self._image_fnames = full_fnames
    ...
    def _load_raw_image(self, raw_idx, retries=10):
        fname = self._image_fnames[raw_idx]
        ...
        if retries > 0:
            try:
                ...
                print(f'Problem loading {raw_idx = }, {retries = }')
                next_idx = (raw_idx + 1) % self.__len__()
                return self._load_raw_image(next_idx, retries=retries-1)
        else:
            print(f'Loading with retries failed')
            return None
